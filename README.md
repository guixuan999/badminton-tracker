...
上传前浏览器会自动截一帧当封面，**不用另外准备图片**。

截哪一帧是有讲究的：**同时取时长的 15% / 30% / 45% / 60% 四个点，各截一张、比平均亮度、留最亮的那张**。

之所以不是固定取某一个点：摄像头刚启动的一两秒画面往往是黑的（自动曝光、对焦还没完成，
编码器关键帧也没就绪），手机拍的片子也常带片头黑场 —— 而固定取点（比如「时长的 10%」）
在短视频上正好落在片头，截出来就是一张全黑的封面。正常视频里这个策略无感，有黑头的会自动避开。

取值范围不出 60% 是为了不落到结尾 —— 有些片子结尾也是黑场。

截不出来（格式解不了）就显示播放占位图，不影响上传。

**已经传上来、封面却是黑的**：进「管理」→ 点那条的「编辑」→ **「重截封面」**。
它会从服务器上取回这段视频、重新走一遍上面的截帧逻辑、覆盖旧封面（旧的会自动清掉）。

> 为什么不在服务端直接重截：服务端是 Python 标准库，解不了 H.264。
> 截帧必须靠浏览器的解码器，所以这一步得在页面里跑。视频是压过的，一般几 MB，取回来很快，
> 过程中会显示取回进度。
...
- **编辑**：改备注、拍摄日期和时间，也可以「换封面」（自己挑一张图）
  或「重截封面」（从服务器上这段视频重新截一帧，用于修掉历史遗留的黑封面）
